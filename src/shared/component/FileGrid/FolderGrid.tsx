...
const FolderGrid = ({ title }: FolderGridProps) => {
  const { isOpen, close, toggle } = useOverlay();

  return (
    <article css={cardStyle}>
      <div css={iconWrapperStyle}>{<IcFolder width={40} height={40} />}</div>
      ...
      <Flex styles={{ width: '100%', justify: 'space-between', align: 'center' }}>
        ...
        <Menu onClose={close}>
          ...
          <MenuList css={{ top: 'calc(100% + 0.4rem)', right: 0, backgroundColor: theme.colors.white }} isOpen={isOpen}>
